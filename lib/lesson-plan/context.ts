...
export type LessonPlanContextError = {
  status: number;
  message: string;
};
...
export async function getLessonPlanContext(): Promise<
  | { ok: true; value: LessonPlanContext }
  | { ok: false; error: LessonPlanContextError }
> {
...
export function createScopedLessonPlanContext(teacherId: string): LessonPlanContext {
  return {
    teacherId,
