...
export default function TikuQuestionDetailModal({
  item,
  onClose,
}: {
      ...
      <div className="relative mx-4 max-h-[85vh] w-full max-w-3xl overflow-y-auto rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_32px_80px_rgba(15,23,42,0.12)] md:mx-0 md:p-8">
        ...
        <div className="flex flex-wrap items-center gap-2 pr-12 text-xs md:pr-14">
          <span className="rounded-full bg-indigo-50 px-2.5 py-1 font-medium text-indigo-700">
            {isZh ? "AP 全局" : "Global AP"}
          </span>
          <span
            className={cn(
              "rounded-full px-2.5 py-1 font-medium",
              tikuDifficultyColors(item.difficulty),
            )}
          >
            {tikuDifficultyLabel(item.difficulty, isZh)}
          </span>
          <span className="rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">
            {tikuCourseLabel(item.course, isZh)}
          </span>
          <span className="rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">
            {isZh ? `单元 ${item.unit}` : `Unit ${item.unit}`}
          </span>
          {item.topic_code ? (
            <span className="rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">
              {item.topic_code}
          ...
          {item.transfer_distance ? (
            <span className="rounded-full bg-blue-50 px-2.5 py-1 font-medium text-blue-700">
              {item.transfer_distance === "near"
                ? (isZh ? "近迁移" : "Near Transfer")
                : (isZh ? "远迁移" : "Far Transfer")}
            </span>
          ) : null}
          {item.standalone_usable ? (
            <span className="rounded-full bg-emerald-50 px-2.5 py-1 font-medium text-emerald-700">
              {isZh ? "可独立使用" : "Standalone"}
            </span>
          ) : null}
        </div>

        {/* Stimulus 信息：有图片时单独显示材料框，无图片时合进题干 */}
        {item.stimulus_id && item.stimulus_image_url && !item.stem?.includes(item.stimulus_image_url) ? (
          <div className="mt-6">
        ...
        {/* 题干：无图片的 stimulus description 合并到题干前面 */}
        <div className="mt-6">
          <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
            {isZh ? "题干" : "Stem"}
          </h3>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {choice.misconception ? (
                          <p className="mt-1 text-xs font-medium text-orange-600">
                            {isZh ? "误解" : "Misconception"}: {choice.misconception}
                          </p>
                        ) : null}
                      </div>
                      {isCorrect ? (
            ...
            <div
              className={cn(
                ...
                item.explanation
                  ...
                  : "border-dashed border-slate-200 bg-slate-50/50 text-slate-400 italic",
              )}
            >
              {item.explanation ? (
                <QuestionContentWithImages
                ...
                isZh ? "暂无解析" : "No explanation available"
              )}
            </div>
          </div>
        ) : null}
        ...
        <div className="mt-6">
          ...
          {item.key_concepts && item.key_concepts.length > 0 ? (
            ...
            <p className="mt-3 text-sm italic text-slate-400">
              {isZh ? "暂无关键概念标签" : "No key concepts tagged"}
            </p>
          )}
        </div>
        ...
        <div className="mt-6 grid gap-4 rounded-[20px] bg-slate-50 p-4 text-sm md:grid-cols-2">
          <div>
            <span className="text-slate-400">{isZh ? "课程" : "Course"}</span>
            <span className="ml-2 font-medium text-slate-700">
              {tikuCourseLabel(item.course, isZh)}
            </span>
          </div>
          <div>
            <span className="text-slate-400">{isZh ? "单元" : "Unit"}</span>
            <span className="ml-2 font-medium text-slate-700">
              {item.unit}
            </span>
          </div>
          {item.topic_code ? (
            <div>
              <span className="text-slate-400">{isZh ? "知识点编号" : "Topic Code"}</span>
              <span className="ml-2 font-medium text-slate-700">
                {item.topic_code}
              </span>
            </div>
          ...
          {item.cognitive_task ? (
            <div>
              <span className="text-slate-400">{isZh ? "认知任务" : "Cognitive Task"}</span>
              <span className="ml-2 font-medium text-slate-700">
                {item.cognitive_task}
              </span>
            </div>
          ...
          {typeof item.similarity === "number" ? (
            <div>
              <span className="text-slate-400">{isZh ? "匹配度" : "Match Score"}</span>
              <span className="ml-2 font-medium text-slate-700">
          ...
          {item.source_assessment ? (
            <div className="md:col-span-2">
              <span className="text-slate-400">{isZh ? "来源文件" : "Source"}</span>
              <span className="ml-2 font-medium text-slate-700">
                {item.source_assessment}
