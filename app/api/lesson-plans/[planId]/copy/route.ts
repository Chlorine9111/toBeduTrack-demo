...
import { copyLessonPlan } from "@/lib/lesson-plan/store";
...
export async function POST(
  ...
  try {
    const lessonPlan = await copyLessonPlan(contextResult.value, parsedParams.data.planId);
    ...
    return jsonError("INTERNAL_ERROR", "复制教案失败", 500);
  }
}
