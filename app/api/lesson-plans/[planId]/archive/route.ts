...
import { archiveLessonPlan } from "@/lib/lesson-plan/store";
...
export async function POST(
  ...
  try {
    await archiveLessonPlan(contextResult.value, parsedParams.data.planId);
    ...
    console.error("归档教案失败", error);
    return jsonError("INTERNAL_ERROR", "归档教案失败", 500);
  }
}
