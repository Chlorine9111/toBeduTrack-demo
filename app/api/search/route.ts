...
import { searchGlobalText } from "@/lib/search/global-search";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const items = await searchGlobalText({
      supabase,
      teacherId,
      query: query.q,
      limit: query.limit,
    ...
    if (error instanceof z.ZodError) {
      return jsonError("VALIDATION_ERROR", "搜索参数不合法", 400, error.flatten());
    }

    console.error("全局文本搜索失败", error);
    return jsonError("INTERNAL_ERROR", "全局文本搜索失败", 500);
  }
}
