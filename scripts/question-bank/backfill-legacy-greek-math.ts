...
const LEGACY_GREEK_MATH_PATTERN =
  /(?:\$[^$\n]*\b(?:alpha|beta|gamma|delta|epsilon|theta|lambda|omega|Delta|Gamma|Theta|Lambda|Sigma|Phi|Psi|Omega|Pi)(?:_[A-Za-z0-9]+)*[^$\n]*\$)|(?:\b(?:alpha|beta|gamma|delta|epsilon|theta|lambda|omega|Delta|Gamma|Theta|Lambda|Sigma|Phi|Psi|Omega|Pi)(?:_[A-Za-z0-9]+)+\b)/;
...
function looksLikeLegacyGreekMath(row: QuestionRow) {
  const combined = [
    row.stem ?? "",
    row.correct_answer ?? "",
    row.explanation ?? "",
  ...
  return LEGACY_GREEK_MATH_PATTERN.test(combined);
}
...
async function main() {
  ...
  while (true) {
    ...
    for (const row of rows) {
      ...
      scanned += 1;
      if (!looksLikeLegacyGreekMath(row)) {
        continue;
      }
...
main().catch((error) => {
  console.error("[backfill-legacy-greek-math] failed");
  console.error(error);
  process.exit(1);
});
